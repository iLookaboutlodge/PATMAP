	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		Try
			...
			If Not IsPostBack Then
				...
				Else
					'hides Subject Municipality label if no Subject has been selected
				...
				'query.CommandText = "SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[liveLTTPhaseIn_" & userID & "]') AND type in (N'U')"
				...
				If Session("showFullTaxClasses") = True Then
					da.SelectCommand = New SqlClient.SqlCommand("select t.taxClassID, t.taxClass, l.show from liveTaxClasses l inner join taxClasses t on l.taxClassID = t.taxClassID INNER JOIN liveLTTtaxClasses_" & userID & " ON liveLTTtaxClasses_" & userID & ".taxClassID = t.taxClassID WHERE t.active = 1 AND liveLTTtaxClasses_" & userID & ".show = 1 AND userID = " & userID, con)
				Else
					da.SelectCommand = New SqlClient.SqlCommand("SELECT DISTINCT m.taxClassID, m.taxClass FROM liveLTTValues v INNER JOIN LTTtaxClasses t ON t.taxClassID = v.taxClassID INNER JOIN LTTmainTaxClasses m ON m.taxClassID = t.LTTmainTaxClassID INNER JOIN liveLTTtaxClasses_" & userID & " l ON l.taxClassID = t.taxClassID WHERE t.active = 1 AND l.show = 1 AND userID = " & userID, con)
				End If
				...
				If Session("showFullTaxClasses") <> True Then
					...
					query.CommandText = "select * from liveTaxClasses where userID = " & Session("userID").ToString & " and taxClassID in (select taxClassID from LTTtaxClasses where LTTmainTaxClassID = 1) and show = 1"
					dr = query.ExecuteReader()
					If dr.Read() Then
					...
					query.CommandText = "select * from liveTaxClasses where userID = " & Session("userID").ToString & " and taxClassID in (select taxClassID from LTTtaxClasses where LTTmainTaxClassID = 2) and show = 1"
					dr = query.ExecuteReader()
					If dr.Read() Then
				...
				If Session("phaseInBaseYearAccess") Then
					...
					If ws.GetItemType(PATMAP.Global_asax.ReportLTTTablesPhaseInFolder) = PATMAPWebService.ItemTypeEnum.Folder Then

						Dim items As PATMAPWebService.CatalogItem() = ws.ListChildren(PATMAP.Global_asax.ReportLTTTablesPhaseInFolder, True)
				...
				Else
					'checks if the folder exists in the report server
					If ws.GetItemType(PATMAP.Global_asax.ReportLTTTablesFolder) = PATMAPWebService.ItemTypeEnum.Folder Then

						Dim items As PATMAPWebService.CatalogItem() = ws.ListChildren(PATMAP.Global_asax.ReportLTTTablesFolder, True)
						Dim catItem As PATMAPWebService.CatalogItem
